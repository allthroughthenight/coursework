...
segment .data
	...
	bin_seperator	db	"' '", 0
	string_prompt	db	"Enter a long string: ", 0
	bin_prompt	db	"Bin: '", 0
	bin_prompt_end	db	"': 0", 0
	min_bin		db	0
	max_bin		db	95

segment	.bss
	bin_size	resb	1	; size of bins
	bin_count	resb	1	; number of bins
	bin_remainder	resb	1	; 'remainder' bin if not a factor of 95
	long_string	resb	99	; long string user input
...
; end after bin range
	...
	; print long string prompt
	mov	eax, string_prompt		; load message address
	call	print_string			; print message

	; take user input for long string
	mov	eax, 0				; clear eax
	mov	ebx, long_string		; load storage address
	call	read_char
	call	read_char

bin_list:
; print out list of bins and their spread

	mov eax, bin_prompt			; load message adderss
	call	print_string			; print message
...
; print out bin of regular numbers
	...
	cmp	cl, [bin_count]			; counter - bin count
	dec	cl				; dec before jump
	jz	bin_list_final			; if zero exit loop
	jmp	after_bin_list			; else repeat
; end after bin list
